...
### キーワード共起ネットワーク（ルールベース）

- 提案JSONからのキーワード抽出・共起ネットワーク生成は **LLMを使わず、ルールベース** で行います。
- 除外したい語句（ストップワード）は `back/app/stopwords_keywords.txt` に1行1語で記述します（`#` 始まりはコメント）。
- 必要に応じて環境変数 `KEYWORD_STOPWORDS_PATH` で別パスのストップワードファイルを指定できます。
